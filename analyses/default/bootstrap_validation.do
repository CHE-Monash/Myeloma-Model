...
* Usage:   stata -b do analyses/default/bootstrap_validation.do [maxbs] [targets] [simdir] [out] [minbs] [stub]
...
*          oos_bootstrap_coverage.csv to analyses/default/results/.
**********
...
*   e.g.  do analyses/default/bootstrap_validation.do 5                 // 5-resample sanity check
*         do analyses/default/bootstrap_validation.do 500 mytargets mysims myout   // HPC paths
local a_maxbs  `"`1'"'
local a_targ   `"`2'"'
local a_simdir `"`3'"'
local a_out    `"`4'"'
...
**********
global bv_targets "analyses/default/targets"                      // observed-target CSVs (movable to HPC)
global bv_simpath  "analyses/default/simulated/outsample/bootstrap"          // the 500 bootstrap sims (live on HPC)
global bv_stub    "all_0_test"                                 // file = <stub>_B<b>.dta
global bv_minbs   "1"
global bv_maxbs   "500"
global bv_out     "analyses/default/results"
...
if `have_curve' {
    ...
    * always written, so the same figure can be redrawn/restyled locally with analyses/default/plot_os_curve.do
    * -- keep the two twoway blocks in sync). CI bounds as lines: validation solid black, simulated
    * short-dash black; the monthly p-value (short-dash) + p=0.05 (solid) are light blue on a light-blue
    * 2nd axis (thin line/ticks).
    ...
    capture {
        twoway ///
          ...
          , ytitle("Overall survival (%)") yscale(range(0 100)) ylabel(0(20)100, angle(0)) ///
            ytitle("p-value", axis(2) color("`pcol'") size(small)) yscale(range(0 1) axis(2) lcolor("`pcol'") lwidth(vthin)) ///
            ylabel(0(.2)1, axis(2) angle(0) format(%2.1f) labcolor("`pcol'") labsize(small) tlcolor("`pcol'") tlwidth(vthin)) ///
