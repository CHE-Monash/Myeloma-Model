...
mata {
	...
	if (rows(idxEligible) > 0) {
		...
		if (rows(idxASCT) > 0) {
			...
			gamma_S1 = bL1F1_CD_S1[1, cols(bL1F1_CD_S1)]
			vRN_ASCT = vRN[idxASCT]
			
			// Calculate survival time using mata function
			vDur_S1 = calcSurvTime(vXB_S1, vRN_ASCT, fbL1F1_CD_S1, gamma_S1)
			...
			if (rows(idxBeyondC1) > 0) {
				// Calculate XB for Spline 2
				pMatrix_C1 = pMatrix[idxBeyondC1, .]
				coef_S2 = bL1F1_CD_S2[1, (1, 2, 3, 5, 6, 8, 9, 11, 12, cols(bL1F1_CD_S2)-1)]'
				vXB_S2 = pMatrix_C1 * coef_S2
				gamma_S2 = bL1F1_CD_S2[1, cols(bL1F1_CD_S2)]
				
				// Conditional RN: survive past C1
				vSurvC1 = exp(-exp(vXB_S2) :* (L1F1_CD_C1 :^ exp(gamma_S2)))
				vRN_S2 = runiform(rows(idxBeyondC1), 1) :* vSurvC1
				
				// Recalculate duration with Spline 2
				vDur_S2 = calcSurvTime(vXB_S2, vRN_S2, fbL1F1_CD_S2, gamma_S2)
				...
				if (rows(idxBeyondC2_local) > 0) {
					...
					// Calculate XB for Spline 3
					pMatrix_C2 = pMatrix[idxBeyondC2, .]
					coef_S3 = bL1F1_CD_S3[1, (1, 2, 3, 5, 6, 8, 9, 11, 12, cols(bL1F1_CD_S3)-1)]'
					vXB_S3 = pMatrix_C2 * coef_S3
					gamma_S3 = bL1F1_CD_S3[1, cols(bL1F1_CD_S3)]
					
					// Conditional RN: survive past C2
					vSurvC2 = exp(-exp(vXB_S3) :* (L1F1_CD_C2 :^ exp(gamma_S3)))
					vRN_S3 = runiform(rows(idxBeyondC2), 1) :* vSurvC2
					
					// Recalculate duration with Spline 3
					vDur_S3 = calcSurvTime(vXB_S3, vRN_S3, fbL1F1_CD_S3, gamma_S3)
		...
		if (rows(idxNoASCT) > 0) {
			...
			gamma_F0 = bL1F0_CD[1, cols(bL1F0_CD)]
			vRN_NoASCT = vRN[idxNoASCT]
			
			// Calculate survival time using mata function
			vDur_F0 = calcSurvTime(vXB_F0, vRN_NoASCT, fbL1F0_CD, gamma_F0)
