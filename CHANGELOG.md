...
- **Maintenance duration: capture and equations** (`prep/data_extraction.do`, `prep/risk_equations.do`, `analyses/default/outcomes/mnr_{full,train}.do`), addressing the maintenance over-costing under Known issues below. `MRDR Long` now carries, per patient, `MND_L1` (maintenance delivered **inside the billed L1 gap**, days — the overlap of every episode with `[DateL1E, gap end]`, so later-line maintenance is excluded), `MNS_L1` (`MND_L1 / TFI_L1`, the `L1_MND` outcome, defined only where an L2 start is observed) and `MNR_L1` (the raw 6-level maintenance drug code). `risk_equations.do` gains **`gen_mnr`**, mirroring `gen_txr`: it collapses `MNR_L1` to the levels each analysis declares in `outcomes/mnr_<coeffs>.do`, so one extraction serves both the historical window the out-of-sample validation scores against and a current-paradigm window. That split is necessary because the maintenance mix stepped abruptly in 2020 (thalidomide 51.6% of starts in 2019, **0** from 2021; lenalidomide 32.5% → ~90%), so the whole-registry mix describes no year that ever existed. Two equations sit beside the `MNT` logit — **`L1_MNR`** (mlogit, year-windowed exactly as `TXR_L1` is) and **`L1_MND`** (betareg on the share) — completing whether / which / how long. Additive: the event skeleton is unchanged and no maintenance events are restored, so every existing equation is untouched. **Not yet consumed by the engine**: `core/process_data.do` L190 still bills `TFI_L1` at blended `cMNT`, the Mata Beta draw is not built, and regimen-specific maintenance pricing is blocked (only lenalidomide has a PBS maintenance listing). Design, evidence, and two corrections to the design's original evidence: `docs/refractory.md` section 7.

### Known issues
- **Maintenance cost is overstated by 69%** (`core/process_data.do` L190). `cost_tx_mnt` bills `cMNT` across the entire `TFI_L1`, i.e. it assumes the patient is on maintenance for the whole L1-to-L2 gap. Against the registry the model bills 61,347 cycles against 36,394 delivered (39% for lenalidomide maintenance alone); about a third of the excess is billed *before* maintenance starts. **Affects the `default` full-pathway analysis and budget-impact work. Does not affect `transport_dvd` or `car_t`** (`$line 2`: maintenance is never costed there). Later-line maintenance (~15% of maintenance months) is not costed at all. The fix (`L1_MNR` mlogit + `L1_MND` beta-regression on the share of `TFI_L1`) is **captured and fitted but not yet wired to the engine** — see Added above; and why full decomposition was rejected: `docs/refractory.md` section 7.
