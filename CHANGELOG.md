...
- **Maintenance duration: capture and equations** (`prep/data_extraction.do`, `prep/risk_equations.do`, `analyses/default/outcomes/mnr_{full,train}.do`), addressing the maintenance over-costing under Known issues below. `MRDR Long` now carries, per patient, `MND_L1` (maintenance delivered **inside the billed L1 gap**, days — the overlap of every episode with `[DateL1E, gap end]`, so later-line maintenance is excluded), `MNT_TTM_L1` (days from L1 end to the first maintenance episode, the offset that makes the maintenance *end* date computable) and `MNR_L1` (the raw 6-level maintenance drug code). `MNS_L1`, the `L1_MND` outcome, is built at fit time as `MND_L1 / (TFI_L1 - TTM)` (the share of the **window**, not of the gap) and is defined only where an L2 start is observed. `risk_equations.do` gains **`gen_mnr`**, mirroring `gen_txr`: it collapses `MNR_L1` to the levels each analysis declares in `outcomes/mnr_<coeffs>.do`, so one extraction serves both the historical window the out-of-sample validation scores against and a current-paradigm window. That split is necessary because the maintenance mix stepped abruptly in 2020 (thalidomide 51.6% of starts in 2019, **0** from 2021; lenalidomide 32.5% → ~90%), so the whole-registry mix describes no year that ever existed. Two equations sit beside the `MNT` logit — **`L1_MNR`** (mlogit, year-windowed exactly as `TXR_L1` is) and **`L1_MND`** (betareg on the share of the window) — completing whether / which / how long. Additive: the event skeleton is unchanged and no maintenance events are restored, so every existing equation is untouched. Now consumed by the engine: see Fixed below. Design, evidence, and two corrections to the design's original evidence: `docs/refractory.md` section 7.

### Known issues
- **No generation model for maintenance refractoriness.** `docs/refractory.md` 4.4 specified a definitional tail rule (`tail = TFI_L1 - TTM - MND_L1 <= 60 days`) at AUC 0.905 against a logit's 0.69. **That validation was contaminated and the rule is withdrawn.** The score also contained the registry's cessation-reason column (`rcess == 5`), IMWG 1.5's *first* limb, which is close to a restatement of the outcome, which the compared logit never saw, and which **the engine cannot compute at all** since reason-for-cessation is not modelled. Re-measured like-for-like on the same 230 patients the tail alone is **0.637**, the logit **0.586**, and `rcess` alone **0.880**; the engine reported **33.7%** flagged against a registry 70.4%, which is the rule working correctly at gap lengths it was never calibrated for. The binding constraint is **selection, not functional form**: a share needs an observed gap *end*, so `L1_MND` is fitted on complete gaps only (median **23.9 months**) against a true median of **73.5 months** for ASCT/CR patients. `MNT_LenRefr_L1` is therefore not emitted by the engine. Three paths forward, and the evidence for each: `docs/refractory.md` 4.4.
...
### Fixed
- **Maintenance cost was overstated by 69%** (`core/process_data.do`). `cost_tx_mnt` billed the blended `cMNT` across the entire `TFI_L1`, i.e. it assumed the patient was on maintenance for the whole L1-to-L2 gap. It now bills the modelled episode: `sim_mnr.do` draws the regimen (`L1_MNR` mlogit, year-windowed as `TXR_L1` is), `sim_mnd.do` draws the duration as a share of the **window** `W = TFI_L1 - TTM(sct)` via `invibeta(mu*phi, (1-mu)*phi, u)` (one uniform per patient, preserving CRN), and `process_data.do` bills `MND_L1 = MNS_L1 x W`. `TTM` is **two** SCT-keyed constants (0.49 / 4.81 months) rather than one: a single pooled 4.30 is ~9x too big for the no-ASCT arm and drives 1.9% of windows negative. On the reference cohort this removes **73.2%** of billed maintenance months (1,552,608 → 896,668), putting `cost_tx_mnt` at $885m (8.7% of treatment cost). Affects the **`default`** full-pathway analysis and budget-impact work; `transport_dvd` and `car_t` are `$line 2` and never costed maintenance. Two limitations remain, both pre-existing: pricing is still the blended `cMNT` (only lenalidomide has a PBS maintenance listing), and **later-line maintenance (~15% of maintenance months) is costed nowhere**. The new `rn_mnr()` / `rn_mnd()` slots re-lay-out `mRN` (`rn_K()` 74 → 76), so patient-level results are not comparable with runs before this change. Evidence, the rejected full decomposition, and why the share is of the window rather than the gap: `docs/refractory.md` section 7.
