...
### Known issues
- **Refractory status is captured in the data but not simulated: the engine wiring is built and held.** Both arms were implemented, run and validated on branch `refractory-status`, and are **not merged**. Treatment-dose generation (`sim_lenrefr.do`, latched 0-to-1 at each line END, definitional on SD/PD else a residual-arm logit) and maintenance-dose generation (`sim_mnt_refr.do`, one draw at L1 end from a logit on `Age Age2 Male i.ECOGcc i.RISS SCT ln(TFI) tail`, AUC 0.79) both work; consumption in the L2-L4 regimen mlogits and OS models is correct, and the OS penalty is real and persists (HR 1.64, no resets, not BCR-mediated). What fails is the **generation mix**, not the mechanism. Simulated maintenance-refractory patients are ~6 years younger than registry ones (63.9 vs 69.9, because maintenance follows transplant) and outnumber the correctly-penalised treatment-refractory group ~3:1, so out-of-sample the refractory *union* has **better** OS than the registry. Treatment refractoriness is simultaneously under-generated (3.9% at L2 vs 8.3% observed) and maintenance over-generated (11.1% vs 7.4%), which is why the union prevalence matches (14.9% vs 15.5%) by offsetting errors. Held rather than dropped: maintenance is clinically the dominant mechanism (70-90% of patients are len-refractory at first relapse in the contemporary era; the registry's 15.5% is an era artefact, and among len-maintenance patients reaching L2 it is 68.6%). This matters for any L2 refractory-targeted analysis. Full diagnosis, evidence and the withdrawn definitional tail rule: `docs/refractory.md` 4.4, 4.7 and 5(6).
